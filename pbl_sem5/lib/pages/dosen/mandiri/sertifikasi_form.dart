...
import 'package:pbl_sem5/widgets/dosen/mandiri/header_sertifikasi_form.dart';
import 'package:pbl_sem5/widgets/dosen/navbar.dart';

const List<String> JENIS_SERTIFIKASI_OPTIONS = ['Keahlian', 'Profesi'];
const List<String> LEVEL_OPTIONS = ['Internasional', 'Nasional'];

class SertifikasiForm extends StatefulWidget {
  final bool isEdit;
  final String? id;
...
class _SertifikasiFormState extends State<SertifikasiForm> {
  ...
  void _loadInitialData() {
    ...
    setState(() {
      ...
      // Selected values
      _selectedJenisSertifikasi = data.jenisSertifikasi;
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      final data = {
        ...
        'sertifikasi_nama': _namaController.text,
        'sertifikasi_start': _tanggalMulai!.toIso8601String(),
        'sertifikasi_end': _tanggalAkhir!.toIso8601String(),
        'sertifikasi_tempat': _tempatController.text,
        'sertifikasi_biaya':
            int.parse(_biayaController.text.replaceAll(RegExp(r'[^0-9]'), '')),
        'sertifikasi_jenis': _selectedJenisSertifikasi,
        'sertifikasi_level': _selectedLevel,
      ...
      if (widget.isEdit && widget.id != null) {
        await _apiMandiriDosen.updateSertifikasi(widget.id!, data);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Sertifikasi berhasil diperbarui')),
          );
          Navigator.pop(context);
        }
      } else {
        await _apiMandiriDosen.createSertifikasi(data);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Sertifikasi berhasil dibuat')),
          );
          Navigator.pop(context);
        }
      }
          ...
          child: Column(
            ...
            children: [
              ...
              _buildDropdown(
                ...
                items: JENIS_SERTIFIKASI_OPTIONS,
                onChanged: (value) =>
                    setState(() => _selectedJenisSertifikasi = value),
              ),
              const SizedBox(height: 16),
              _buildDropdown(
                label: 'Level',
